...
public class ProfissionalDao {
    ...
    public String inserir(Profissional profissional) throws SQLException, ClassNotFoundException {
        try (Connection conn = new ConexaoFactory().conexao();
             PreparedStatement stmt = conn.prepareStatement(
                     "INSERT INTO T_FIAP_PROFISSIONAIS (nome, cpf, cro, especializacao, senha) VALUES (?,?,?,?,?)"
             )) {
    ...
    public Profissional login(String cro, String senha) throws SQLException, ClassNotFoundException {
        try (Connection conn = new ConexaoFactory().conexao();
             PreparedStatement stmt = conn.prepareStatement(
                     "SELECT * FROM T_FIAP_PROFISSIONAIS WHERE CRO = ? AND SENHA = ?"
             )) {
